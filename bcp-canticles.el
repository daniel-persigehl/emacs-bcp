;;; bcp-canticles.el --- Liturgical canticle texts -*- lexical-binding: t -*-
...
;;
;; User configuration:
;;   `bcp-canticle-language'        — default language ('english or 'latin)
;;   `bcp-canticle-overrides'       — per-canticle language overrides
;;   `bcp-canticle-append-gloria'   — append Gloria Patri after canticles
...
(defconst bcp-canticle-gloria-patri-english
  "Glory be to the Father, and to the Son : and to the Holy Ghost;\n\
   As it was in the beginning, is now, and ever shall be : world without end.  Amen."
  "Gloria Patri in English (BCP pointing).")

(defconst bcp-canticle-gloria-patri-latin
  nil
...
`bcp-canticle-append-gloria' is non-nil.
...
(defun bcp-canticle-data (symbol)
  "Return the data plist for canticle SYMBOL, or nil."
  (cdr (assq symbol bcp-canticle-texts)))

(defun bcp-canticle-effective-language (symbol)
  "Return the effective language for canticle SYMBOL.
Checks `bcp-canticle-overrides' first, then `bcp-canticle-language'."
  (or (cdr (assq symbol bcp-canticle-overrides))
      bcp-canticle-language))

(defun bcp-canticle-get (symbol &optional language)
...
Returns a string or nil."
  (let* ((data (bcp-canticle-data symbol))
         (lang (or language (bcp-canticle-effective-language symbol)))
...
(defun bcp-canticle-title (symbol)
  "Return the display title for canticle SYMBOL."
  (plist-get (bcp-canticle-data symbol) :title))

(defun bcp-canticle-source (symbol)
  "Return the scriptural or patristic source description for canticle SYMBOL."
  (plist-get (bcp-canticle-data symbol) :source))

(defun bcp-canticle-gloria-p (symbol)
  "Return non-nil if Gloria Patri should follow canticle SYMBOL."
  (plist-get (bcp-canticle-data symbol) :gloria))

(defun bcp-canticle-gloria-text (&optional language)
  "Return the Gloria Patri text in LANGUAGE (default: effective language)."
  (let ((lang (or language (bcp-canticle-effective-language 'gloria-patri))))
    (or (plist-get (bcp-canticle-data 'gloria-patri) (intern (format ":%s" lang)))
        (plist-get (bcp-canticle-data 'gloria-patri) :english))))


(provide 'bcp-canticles)
;;; bcp-canticles.el ends here
